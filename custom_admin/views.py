...
from custom_admin.models import Categories,ProductAttribute,ProductAttributeValues,Product,ProductImage, ProductAttributeAssoc
...
def CategoryUpdate(request, id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('custom_admin:CategoryList')
    else:
...
def CategoryDelete(request, id):
    data = get_object_or_404(Categories, id=id)
    try:
        data.delete()
        messages.success(request, "Category deleted succesfully")
    except IntegrityError:
        messages.warning(request, "Category cant deleted because of integrity")
...
def product_add(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and formset.is_valid() and formset2.is_valid():
            ...
            return redirect('custom_admin:product')
